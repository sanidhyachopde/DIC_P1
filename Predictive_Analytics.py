...
def RandomForest(X_train,Y_train,X_test):
        ...
        class DecisionTree():
          ...
          def find_varsplit(self):
            for var_idx in range(self.c): self.find_better_split(var_idx)
            ## Ready to split 
            ...
            self.lhs = DecisionTree(self.x,self.y,idxs=self.idxs[lhs],min_leaf=self.min_leaf)
            self.rhs = DecisionTree(self.x,self.y,idxs=self.idxs[rhs],min_leaf=self.min_leaf)
          def find_better_split_slow(self,var_idx):
            ...
            for val in x:
              ...
              ## Using Mse
              lhs_mse = mean_squared_error(y[lhs],[y[lhs].mean() for i in range(y[lhs].shape[0])])
              rhs_mse = mean_squared_error(y[rhs],[y[rhs].mean() for i in range(y[rhs].shape[0])])
              # curr_score = np.array([lhs_mse,rhs_mse]).mean()
              
              if curr_score < self.score:
                self.var_idx, self.score, self.split_on, self.var_name = var_idx, curr_score, val , self.x.columns[var_idx]
          ...
          def find_better_split(self,var_idx):
            ...
            for i in range(self.n-self.min_leaf-1):
              ...
              if curr_score < self.score:
                self.var_idx, self.score, self.split_on, self.var_name = var_idx, curr_score, xi , self.x[var_idx]
...
def PCA(X_train,N):
    ...
    components = eigenvectors[0:N]
    X_projected = np.dot(X_train, components.T)
    return -X_projected
